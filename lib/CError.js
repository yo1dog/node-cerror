...
const origStackSymbol = Symbol('origStack');
CError.causeSymbol = causeSymbol;
CError.origStackSymbol = origStackSymbol;
...
function linkErrors(cause, result) {
  ...
  if (stackPropDesc) {
    ...
    ExtendableError.setUnenumerable(result, CError.origStackSymbol, undefined);
  }
  
  // create a getter for the stack
  /** @type {any} */(result).stack = null;
  Object.defineProperty(result, 'stack', {
    get: function get() {
      ...
      if (this instanceof Error) {
        stack += /** @type {any} */(this)[CError.origStackSymbol];
      }
      else {
 ...
 * @template T
 * @param {T} val 
 * @returns {val is T & {[CError.causeSymbol]: unknown}}
 */
function canHoldCause(val) {
  return (
    typeof val === 'object' &&
    val &&
    CError.causeSymbol in val
  );
}
